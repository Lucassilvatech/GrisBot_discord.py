...
class Embeds(commands.Cog):
    ...
    async def sobre_gris(self, ctx, member: self.discord.Member = None):
        grisEmbed = self.discord.Embed(title="Olá eu sou a Gris", description="Sou a bot mais fofa que existe nesse mundo!", color=0xfa0000)
        grisEmbed.set_author(name=f"{self.bot.user.name}", icon_url=f"{self.bot.user.avatar_url}")
        ...
        grisEmbed.set_image(url=f"{self.bot.user.avatar_url}")
